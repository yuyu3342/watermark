...
const App = () => {
  ...
  const handleFillCanvas = () => {
    ...
    if (activeLayer.type === 'text') {
        ...
        if (logoData) {
            const imgRatio = logoData.imgObject.width / logoData.imgObject.height;
            newSize = imgRatio > canvasRatio ? (1 / canvasRatio * imgRatio) * 1000 : 1000;
        }
    }
    updateLayer(activeLayer.id, { size: Math.ceil(newSize * 1.1), posX: 50, posY: 50, rotation: 0, isTiled: false, opacity: 1 });
  ...
  const saveMaskState = () => {
      ...
      setMaskHistory(prev => [...prev.slice(-4), imgData]); 
  };
  ...
  const applySmartRepair = async () => {
     ...
     try {
       ...
       if (!hasMask) {
           setIsProcessing(false); setLoadingMsg(""); alert("请先涂抹需要去除的区域"); return;
       }

       const padding = 10;
       minX = Math.max(0, minX - padding); maxX = Math.min(w - 1, maxX + padding);
       minY = Math.max(0, minY - padding); maxY = Math.min(h - 1, maxY + padding);
       ...
       newImg.onload = () => {
           ...
           newList[selectedIndex] = { ...newList[selectedIndex], src: newSrc, imgObject: newImg };
         ...
         console.error(err);
         setIsProcessing(false); setLoadingMsg(""); alert("修复出错，请重试");
     }
  };
  ...
  const detectFaces = async () => {
    ...
    if (!faceMeshRef.current || !currentImg || !maskCanvasRef.current) {
        if (!faceMeshRef.current) alert("AI 引擎初始化中，请稍后再试...");
        return;
    }
    ...
    try {
        faceMeshRef.current.onResults((results) => {
            setIsAiLoading(false); setLoadingMsg("");
        ...
        setIsAiLoading(false); setLoadingMsg("");
        console.error(err); alert("AI 识别失败，请检查网络");
    }
  };
  ...
  const handleDrawStart = (x, y) => {
      ...
      ctx.strokeStyle = 'rgba(255, 0, 0, 0.5)'; ctx.lineWidth = brushSize * scaleX; 
      ctx.beginPath(); ctx.moveTo(drawX, drawY); ctx.lineTo(drawX, drawY); ctx.stroke();
  };
                                ...
                                {activeLayer.type === 'text' ? (
                                    <>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <input type="range" min="0" max="20" value={activeLayer.strokeWidth} onChange={e => updateLayer(activeLayer.id, {strokeWidth: Number(e.target.value)})} className="flex-1 h-1 bg-gray-700 rounded accent-blue-500"/>
                                            {activeLayer.strokeWidth > 0 && <input type="color" value={activeLayer.strokeColor} onChange={e => updateLayer(activeLayer.id, {strokeColor: e.target.value})} className="h-6 w-6 bg-transparent border-none"/>}
                                        </div>
                                    </>
                                ) : (
